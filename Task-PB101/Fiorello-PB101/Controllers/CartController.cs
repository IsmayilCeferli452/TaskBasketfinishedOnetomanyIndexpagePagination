...
namespace Fiorello_PB101.Controllers
{
    public class CartController : Controller
    {
        ...
        [HttpPost]
        public IActionResult DecreaseProductCountInBasket(int? id)
        {
            if (id is null) return BadRequest();
            ...
            var basketData = basketDatas.FirstOrDefault(m => m.Id == id);

            if (basketData != null && basketData.Count != 0)
            {
                basketData.Count--;
            }
            else
            {
                basketDatas.Remove(basketData);
            }
        ...
        [HttpPost]
        public IActionResult IncreaseProductCountInBasket(int? id)
        {
            if (id is null) return BadRequest();
